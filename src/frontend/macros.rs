...
fn extract_pattern<'a>(ast: &Ast<'a>, has_var_pat: bool) -> PatternNode<'a> {
    match ast {
        ...
        Ast::SExpr(_, v) => {
            if !v.is_empty()
                && matches!(&v[0], Ast::Symbol(_, v) if v.len() == 1 && v[0] == "..")
                && !has_var_pat
            {
...
fn find_macros<'a>(ast: &Ast<'a>, macros: &mut HashMap<String, Macros<'a>>) {
    match ast {
        ...
        Ast::SExpr(_, v) => {
            if v.len() >= 3
                && matches!(v.get(0), Some(Ast::Symbol(_, v)) if v.len() == 1 && v[0] == "defmacro")
            {
...
fn apply_macros<'a>(ast: &mut Ast<'a>, macros: &HashMap<String, Macros<'a>>) -> bool {
    match ast {
        ...
        Ast::SExpr(_, v)
            if !v.is_empty()
                && !matches!(&v[0], Ast::Symbol(_, v) if v.len() == 1 && v[0] == "defmacro") =>
        {
            let mut b = false;
            for v in v.iter_mut() {
            ...
            match v.get(0) {
                Some(Ast::Symbol(_, macro_name))
                    if macro_name.len() == 1 && macros.contains_key(macro_name[0]) =>
                {
...
fn perform_post_macro_transformations(ast: &mut Ast<'_>) {
    match ast {
        ...
        Ast::SExpr(s, v)
            if v.len() == 2
                && matches!(v.get(0), Some(Ast::Symbol(_, v)) if v.len() == 1 && v[0] == "#quote") =>
        {
